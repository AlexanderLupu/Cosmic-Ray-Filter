...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 20 11:15:35 2025
...
WIDTH_SPACE_THRESHOLD = 40  # Min spacing (samples) between peaks in a single channel
...
DATA_FOLDER = "/Users/alexlupu/github/dune-apex-umn/DATA/20230722/jsonData/"  # Folder with .json data
    ...
    for chn in range(NCC):
        ...
        if len(pks) > 1:
            ...
            if np.any(np.diff(pks_sorted) < WIDTH_SPACE_THRESHOLD):
                if debug:
                    print(f"{output_align}[DEBUG] ch{chn}: two peaks < {WIDTH_SPACE_THRESHOLD}=> exclude event.")
                return False
            
        if peak_width > WIDTH_PEAK_THRESHOLD:
            if debug:
                print(f"{output_align}[DEBUG] ch{chn}: Width of peak > ")
            return False
...
def show_event(adc, run_time, event_id,
    ...
    saved to 'saveFileName'. Additionally, writes a CSV line with metadata:
    #Folder_name, EvtID, Raw_file_nr, Raw_Evt_nr, File_nr, Evt_nr
    """

    # -------------------------------------------
    # 1) Build a descriptive string (for the plot title)
    # -------------------------------------------
    ...
    # -------------------------------------------
    # 2) Write a line in plot_names.csv
    # -------------------------------------------
    csv_file_dir = "/Users/alexlupu/github/dune-apex-umn/get-started-2025/valid-cosmic-rays"
    Path(csv_file_dir).mkdir(parents=True, exist_ok=True)  # Create folder if needed
